...
async function loadStartingProcesses() {
    try {
        ...
        // Calculate total claims for the START node
        const totalClaims = data.reduce((sum, proc) => sum + proc.count, 0);
        
        // Create the START root node
        const startNode = {
            id: 'root_start',
            name: 'START',
            count: totalClaims,
        ...
        // Position starting nodes in a row below the START node
        const totalNodes = data.length;
        const spacing = Math.min(200, width / (totalNodes + 1));
        const startY = 280;
        
        const startingNodes = data.map((proc, i) => ({
            id: `start_${proc.process}`,
            name: proc.process,
            count: proc.count,
            percentage: proc.percentage,
            avgDuration: proc.avg_duration || 0,
            depth: 1,
            path: [proc.process],
            isStarting: true,
            isTermination: false,
            parentId: 'root_start',
            radius: 30 + (proc.count / totalClaims) * 25,
        ...
        // Create links from START to all starting processes
        const startLinks = startingNodes.map(node => ({
            source: 'root_start',
            target: node.id,
            isTermination: false
        }));
        
        currentNodes = [startNode, ...startingNodes];
        nodes = [...currentNodes];
        links = [...startLinks];
        
        updateVisualization();
        updateBreadcrumb('START → Click on a process to explore');
    } catch (error) {
...
async function expandNode(node) {
    if (node.isTermination || node.isRoot) return;
            ...
            flowData.next_steps.forEach((step, index) => {
                ...
                if (!nodes.find(n => n.id === childId)) {
                    const childNode = {
                        ...
                        avgDuration: step.avg_duration || 0,
                        depth: childDepth,
...
function updateVisualization() {
    ...
    nodeEnter.append('circle')
        ...
        .attr('r', d => d.radius)
        .attr('fill', d => {
            if (d.isRoot) return '#667eea';
            return COLORS[d.name] || '#95A5A6';
        })
        .attr('stroke', d => {
            if (d.isRoot) return '#764ba2';
            if (d.isTermination) return '#C0392B';
            return 'white';
        })
        .attr('stroke-width', d => d.isRoot ? 4 : 3)
        .style('cursor', d => (d.isTermination || d.isRoot) ? 'default' : 'pointer')
        .on('click', (event, d) => {
            event.stopPropagation();
            if (!d.isRoot) expandNode(d);
        })
    ...
    nodeEnter.append('text')
        ...
        .attr('fill', d => d.isRoot ? '#667eea' : '#2C3E50')
        .attr('font-size', d => d.isRoot ? '14px' : '11px')
        .attr('font-weight', d => d.isRoot ? 'bold' : '500')
        .attr('pointer-events', 'none')
        .text(d => d.name);

    // Add average duration below the label (if present and not root)
    nodeEnter.filter(d => !d.isRoot)
        ...
        .attr('pointer-events', 'none')
        .text(d => d.avgDuration ? `${d.avgDuration}m` : '');
...
function showTooltip(event, d) {
    const pathStr = d.isTermination 
        ...
        : (d.isRoot ? 'START' : d.path.join(' → '));
    
    let percentText = '';
    if (typeof d.percentage === 'number') {
        percentText = `${d.percentage.toFixed(1)}%`;
    }

    let avgText = '';
    if (typeof d.avgDuration === 'number' && d.avgDuration > 0 && !d.isRoot) {
        avgText = `${d.avgDuration.toFixed(2)} minutes`;
    }

    let html = `<div class="tooltip-title">${d.name}</div>`;
    html += `<div class="tooltip-row"><span class="tooltip-label">Claims:</span><span>${d.count}</span></div>`;
    if (percentText) html += `<div class="tooltip-row"><span class="tooltip-label">Percentage:</span><span>${percentText}</span></div>`;
    if (avgText) html += `<div class="tooltip-row"><span class="tooltip-label">Avg duration:</span><span>${avgText}</span></div>`;
    html += `<div class="tooltip-row"><span class="tooltip-label">Path:</span><span>${pathStr}</span></div>`;
    
    if (d.isRoot) {
        html += '<div class="tooltip-hint">Root node - all claims start here</div>';
    } else if (d.isTermination) {
        html += '<div class="tooltip-hint">Termination point</div>';
    } else {
