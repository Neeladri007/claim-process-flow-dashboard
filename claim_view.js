// Load claim numbers when page loads
document.addEventListener('DOMContentLoaded', loadClaimNumbers);

async function loadClaimNumbers() {
    try {
        const response = await fetch('http://localhost:8000/api/claim-numbers');
...
async function searchClaim() {
    ...
    try {
        const response = await fetch(`http://localhost:8000/api/claim-path/${claimNumber}`);
